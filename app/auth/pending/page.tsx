...
export default function PendingVerificationPage() {
  ...
  const [cooldown, setCooldown] = useState(60); // seconds until resend allowed
  ...
  async function resend() {
    try {
      setErr('');
      // Supabase v2 resend endpoint
      const { error } = await supabase.auth.resend({
        type: 'signup',
        email,
      });
      if (error) {
        // Common Supabase message: “you can only request this after N seconds”
        setErr(error.message || 'Unable to resend at the moment.');
        // If server provides a time hint, keep our cooldown UX
        return;
      }
      setCooldown(60);
    } catch (e: any) {
